...
async function persistToSupabase(result: ProspectResult, runId: string): Promise<void> {
  ...
  const prospectBody = {
    ...
    tier: 'A',
    campaign: 'P2',
    // Either tiered judge (items 7+8) set send_status='flag' for borderline/dissenting cases,
    // OR email-find flag-status pattern (item 6) set flag_status=true for red/no-email cases.
    // Both paths converge to 'flag' status; otherwise 'pending'.
    send_status: result.send_status || (result.flag_status ? 'flag' : 'pending'),
    assigned_ae: result.ae.name,
    icp_status: result.icp_verdict,
    icp_reason: result.icp_reason || '',
    icp_type: icpType,
